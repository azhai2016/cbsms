...
function print_sales_orders() {
	...
	for ($i = $from; $i <= $to; $i++) {
		...
		if ($i == $from || $email == 1)
			$rep = new FrontReport(_('销售订单报表'), 'SalesOrderBulk', user_pagesize(), 9, $orientation);
		if ($print_as_quote == 1) {
			$rep->title = _('报价');
		...
		$rep->TextCol(3, 6, _('小计'), -2);
		$rep->TextCol(6, 7,	$DisplaySubTot, -2);
		$rep->NewLine();
		if ($myrow['freight_cost'] != 0.0) {
			$DisplayFreight = number_format2($myrow['freight_cost'], $dec);
			$rep->TextCol(3, 6, _('快递'), -2);
			$rep->TextCol(6, 7,	$DisplayFreight, -2);
			$rep->NewLine();
		}	
		$DisplayTotal = number_format2($myrow['freight_cost'] + $SubTotal, $dec);
		if ($myrow['tax_included'] == 0) {
			$rep->TextCol(3, 6, _('金额合计'), - 2);
		...
		foreach($tax_items as $tax_item) {
			...
			if ($myrow['tax_included']) {
				if ($SysPrefs->alternative_tax_include_on_docs() == 1) {
					...
					$rep->TextCol(3, 7, _('包含').' '.$tax_type_name.' '._('金额').': '.$DisplayTax, -2);
			}
			else {
