...
function getTransactions($from, $to) {
	$sql = "SELECT bom.parent,
			bom.component,
			concat(item.item_name,'_',item.item_spec,'_',item.item_factory) as CompDescription,
			bom.quantity,
			bom.loc_code,
			bom.workcentre_added
		FROM "
			.TB_PREF."item_codes item,"
			.TB_PREF."bom bom
		WHERE item.id=bom.component
...
function print_bill_of_material() {
	...
	$headers = array(_('配件/原料'), _('名称'), _('货位'), _('生产车间'), _('数量'));

	$aligns = array('left',	'left',	'left', 'left', 'right');

	$params =   array( 	0 => $comments,
						1 => array('text' => _('配件/原料'), 'from' => $frompart, 'to' => $topart));

	$rep = new FrontReport(_('BOM清单列表'), 'BillOfMaterial', user_pagesize(), 9, $orientation);
	if ($orientation == 'L')
