...
function print_work_order_listing() {
	...
	$open = $open_only == 1 ? _('是') : _('否');
	
	$cols = array(0, 100, 120, 165, 210, 275, 315, 375, 385, 440, 495, 555);

	$headers = array(_('类型'), '#', ('工单号'), _('货位'), _('商品'), _('需求'), _('完成'), ' ', _('日期'), _('需求日期'), _('关闭'));
	...
	if ($res->num_rows==0) {
		display_warning(_('没有查询该商品的配件列表'));		
	    return;
	}
	...
	$params =   array( 	0 => $comments,
						1 => array('text' => _('商品'), 'from' => $items, 'to' => ''),
						2 => array('text' => _('货位'), 'from' => $loc, 'to' => ''),
						3 => array('text' => _('完成状态'), 'from' => $open, 'to' => ''),
					);

	$rep = new FrontReport(_('工单列表'), 'WorkOrderListing', user_pagesize(), 9, $orientation);
	if ($orientation == 'L')
